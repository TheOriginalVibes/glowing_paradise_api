...
public class BaseCommand implements CommandExecutor {
    ...
    private String _w(String key, Object... params) {
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        if (subCommand != null) {
            ...
            if (permission != null && !sender.hasPermission(permission)) {
                sender.sendMessage(_w("command_permdenied"));
            }else {
                try {
                    ...
                    sender.sendMessage(_w("command_internalerror"));
                    t.printStackTrace(System.err);
                }
            }
        }else {
            sender.sendMessage(_w("command_notfound"));
        }
        return true;
    }
    ...
    public void helpCommand(CommandSender sender, CommandArgs args) {
        if (args.size() > 0) {
            ...
            if (command != null) {
                ...
                sender.sendMessage("    " + _w(command.getName() + "_description"));
            }else {
                sender.sendMessage(_w("help_cmdnotfound", args.getString(0)));
            }
        }else {
            sender.sendMessage(_w("help_listofcommands"));
            sender.sendMessage(" ");

            List<SubCommand> subCommands = new ArrayList<SubCommand>(args.getBaseCommand().getAllSubCommands());
            subCommands.sort(SubCommand.COMPARATOR);
    ...
    private void versionCommand(CommandSender sender, CommandArgs args) {
        sender.sendMessage(_w("version_pluginversion", this.plugin.getDescription().getVersion()));
        sender.sendMessage(" ");
    }
    ...
    private void reloadCommand(CommandSender sender, CommandArgs args) {
        ...
        sender.sendMessage(_w("reload_completed"));
    }
    ...
    private void languageCommand(CommandSender sender, CommandArgs args) {
        if (args.size() > 0) {
            ...
            if (tranlation != null) {
                ...
                plugin.saveConfig();
                sender.sendMessage(_w("language_changed", _w("language_" + tranlation.getLanguage())));
            }else {
                sender.sendMessage(_w("language_failed", language));
            }
        }else {
            sender.sendMessage(_w("language_current", _w("language_" + this.plugin.getTranslation().getLanguage())));
        }
    }

    //@Command
    @RequiresPermission("glowingparadise.libminecraft")
