package at.vibes.libInventorys.manager;

import at.vibes.libInventorys.content.InventoryContents;
import at.vibes.libInventorys.listener.InventoryListener;
import at.vibes.libInventorys.opener.ChestInventoryOpener;
import at.vibes.libInventorys.opener.InventoryOpener;
import at.vibes.libInventorys.opener.SpecialInventoryOpener;
...
public class InventoryManager {
    ...
    private final Map<UUID, LibInventory> inventories;
    ...
    public List<Player> getOpenedPlayers(LibInventory inv) {
    ...
    public Optional<LibInventory> getInventory(Player p) {
        return Optional.ofNullable(this.inventories.get(p.getUniqueId()));
    }

    public void setInventory(Player p, LibInventory inv) {
        if (inv == null)
    ...
    public void handleInventoryOpenError(LibInventory inventory, Player player, Exception exception) {
        inventory.close(player);

        Bukkit.getLogger().log(Level.SEVERE, "Error while opening LibInventory:", exception);
    }

    public void handleInventoryUpdateError(LibInventory inventory, Player player, Exception exception) {
        inventory.close(player);

        Bukkit.getLogger().log(Level.SEVERE, "Error while updating LibInventory:", exception);
    }
    ...
    class InvListener implements Listener {
        ...
        public void onInventoryClick(InventoryClickEvent e) {
            ...
            if (clickedInventory == p.getOpenInventory().getTopInventory()) {
                ...
                LibInventory inv = inventories.get(p.getUniqueId());
